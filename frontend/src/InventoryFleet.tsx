...
export default function InventoryFleet({token,open}:Props){
  ...
  return <div className="sentinel-fleet">
    ...
    <section><h3 className="fleet-section-title">Fleet Summary</h3><div className="fleet-kpis">
      <article className="wide"><Server/><span>Total Devices</span><strong>{data.total}</strong><small>Local controlled inventory</small></article>
      ...
      <article><span>YTD Availability</span><strong className="pending">Baseline pending</strong></article>
      <article><span>Below SLA</span><strong className="pending">Insufficient evidence</strong></article>
    ...
    <section><div className="fleet-section-head"><h3 className="fleet-section-title">Compliance Trends</h3><span className="confidence"><i/> Coverage: Baseline pending</span></div><div className="trend-panel"><div><span>Availability vs Target</span><span>12W Trend</span></div><div className="trend-grid"><div className="trend-empty"><Activity size={22}/><b>Baseline pending</b><small>Historical availability collection is required before a governed SLA trend can be displayed.</small></div></div></div></section>
    <section><div className="fleet-section-head fleet-status-head"><h3 className="fleet-section-title">Fleet Status</h3><div><button aria-label="Apply filters" onClick={()=>load()}><Filter size={15}/></button><button aria-label="Export current view" onClick={exportCsv}><Download size={15}/></button></div></div>
      ...
      <div className="fleet-device-list">{data.items.map((d:any)=>{const uptime=uptimes[d.id];const matched=d.logicmonitor_match_status==='Matched';return <article key={d.id} className={!matched?'has-gap':''}><div className="device-card-top"><button className="fleet-device-name" disabled={opening===d.id} onClick={()=>showDetail(d)}><Server size={19}/>{opening===d.id?'Loading details…':d.generated_name}</button><span className={'fleet-state '+(matched?'healthy':'pending')}>{matched?'Monitored':'Mapping pending'}</span></div><div className="device-card-grid"><div><span>Management IP</span><b>{d.management_ip||'Mapping pending'}</b></div><div><span>Site</span><b>{d.site_code} · {d.city}</b></div><div><span>Region</span><b>{d.province_region||'Not monitored'}</b></div><div><span>Current Uptime</span><b className={uptime?.state==='failed'?'danger':''}>{uptime?.display||'Collection pending'}</b></div><div><span>WTD Availability</span><b className="pending">Baseline pending</b></div><div><span>YTD Availability</span><b className="pending">Baseline pending</b></div></div></article>})}</div>
