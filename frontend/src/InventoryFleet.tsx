...
export default function InventoryFleet({token,open}:Props){
  ...
  const[q,setQ]=useState('');const[site,setSite]=useState('');const[slaSummary,setSlaSummary]=useState<any>(null);
  ...
  return <div className="sentinel-fleet">
    ...
    <section><div className="fleet-section-head"><h3 className="fleet-section-title">Compliance Trends</h3><span className="confidence"><i/> Coverage: {slaSummary?slaSummary.fleet_ytd.coverage.toFixed(1)+'%':'Baseline pending'}</span></div><div className="trend-panel"><div><span>Availability vs Target</span><span>12W Trend</span></div><div className="trend-grid"><div className="trend-empty"><Activity size={22}/><b>Baseline pending</b><small>Historical availability collection is required before a governed SLA trend can be displayed.</small></div></div></div></section>
