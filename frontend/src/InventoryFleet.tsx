...
export default function InventoryFleet({token,open}:Props){
  ...
  const[q,setQ]=useState('');const[site,setSite]=useState('');const[slaSummary,setSlaSummary]=useState<any>(null);const[trend,setTrend]=useState<any>(null);const[pageSize,setPageSize]=useState(10);
  ...
  return <div className="sentinel-fleet">
    ...
    <section><div className="fleet-section-head fleet-status-head"><h3 className="fleet-section-title">Fleet Status<Help text="Every switch in inventory with its site, management IP, current uptime, and week-to-date / year-to-date availability. Filter by search or site code; click a device name to open its full LogicMonitor detail (health, interfaces, neighbors, OSPF). WTD/YTD show 'Insufficient evidence' when coverage is below 90%."/></h3><div><button aria-label="Apply filters" onClick={()=>load()}><Filter size={15}/></button><button aria-label="Export current view" onClick={exportCsv}><Download size={15}/></button></div></div>
      ...
      <div className="fleet-device-list">{data.items.map((d:any)=>{const uptime=uptimes[d.id];const matched=d.logicmonitor_match_status==='Matched';const s=slaByLm[d.logicmonitor_device_id];return <article key={d.id} className={!matched?'has-gap':''}><div className="device-card-top"><button className="fleet-device-name" disabled={opening===d.id} onClick={()=>showDetail(d)}><Server size={19}/>{opening===d.id?'Loading details…':d.generated_name}</button><span className={'fleet-state '+(matched?'healthy':'pending')}>{matched?'Monitored':'Mapping pending'}</span></div><div className="device-card-grid"><div><span>Management IP</span><b>{d.management_ip||'Mapping pending'}</b></div><div><span>Site</span><b>{d.site_code} · {d.city}</b></div><div><span>Region</span><b>{d.province_region||'Not monitored'}</b></div><div><span>Current Uptime</span><b className={uptime?.state==='failed'?'danger':''}>{uptime?.display||'Collection pending'}</b></div><div><span>Rolling 7-day</span><b className={s?.rolling_7?.availability!=null?'':'pending'}>{fmt(s?.rolling_7)}</b></div><div><span>Week-to-date</span><b className={s?.wtd?.availability!=null?'':'pending'}>{fmt(s?.wtd)}</b></div><div><span>Previous week</span><b className={s?.prev_week?.availability!=null?'':'pending'}>{fmt(s?.prev_week)}</b></div><div><span>YTD Availability</span><b className={s?.ytd?.availability!=null?'':'pending'}>{fmt(s?.ytd)}</b></div></div></article>})}</div>
      {!data.items.length&&!loading&&<div className="fleet-empty">No inventory devices match the selected filters.</div>}<div className="fleet-pagination" style={{justifyContent:'space-between'}}><label className="ds-pager-size">Show <select value={pageSize} onChange={e=>{const s=Number(e.target.value);setPageSize(s);load(1,s)}}>{[10,20,50,100].map(s=><option key={s} value={s}>{s}</option>)}</select> per page</label><div style={{display:'flex',alignItems:'center',gap:12}}><span>Page {data.page} of {data.pages} · {data.total} total</span><button disabled={data.page<=1} onClick={()=>load(data.page-1)}>Previous</button><button disabled={data.page>=data.pages} onClick={()=>load(data.page+1)}>Next</button></div></div>
    </section>
