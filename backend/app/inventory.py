...
def inventory_devices(q:str="",site:str|None=None,city:str|None=None,zone:str|None=None,device_type:str|None=None,role:str|None=None,model:str|None=None,match_status:str|None=None,criticality:str|None=None,enabled:bool|None=None,sort:str="generated_name",direction:str="asc",page:int=Query(1,ge=1),page_size:int=Query(25,ge=1,le=200),user=Depends(current_user),db:Session=Depends(session)):
    ...
    evidence=[switch_evidence(db,d) for d in all_rows]
    counts=[count or 1 for count,_ in evidence]
    total_physical=sum(counts)
    return {"items":[device_json(x,db) for x in all_rows[start:start+page_size]],"total":len(all_rows),"total_physical":total_physical,"total_stacks":sum(1 for count in counts if count>1),"stack_members":sum(count for count in counts if count>1),"page":page,"page_size":page_size,"pages":max(1,(len(all_rows)+page_size-1)//page_size)}
